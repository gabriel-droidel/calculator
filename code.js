...
keys.forEach((key) =>
	key.addEventListener('click', () =>
		processParsedNumber(numbers, key.textContent)
	)
); // assign value of the key inside the data object

// Keyboard support
document.body.addEventListener('keydown', (event) =>
	handleKeyboardInput(numbers, event.key)
);
...
operators.forEach((operator) =>
	operator.addEventListener('click', () =>
		assignOperator(numbers, operator.textContent)
	)
); // assign operator inside the data object
...
function processParsedNumber(numbers, value) {
	...
	if (numbers.displayed.length < 6) numbers.displayed += value;
...
function assignValue(numbers) {
	...
	if (numbers.displayed != '') {
		...
		if (numbers.result != null && numbers.operation) {
			// if there is a second number and operation
			numbers.first = +numbers.displayed;
		} else if (numbers.operation && numbers.first != null)
			numbers.second = +numbers.displayed;
		// for already chosen operator, the second number gets the value
		else numbers.first = +numbers.displayed;
	}
...
function addDecimal(numbers) {
	...
	if (!numbers.decimal) {
		if (numbers.displayed === '') numbers.displayed = '0.';
		else {
			numbers.displayed += '.';
		}
...
function divide(firstNumber, secondNumber, numbers) {
	...
	if (secondNumber === 0) {
		...
		return 'ERROR';
	} else return +(firstNumber / secondNumber).toFixed(2);
}
...
function evaluate(numbers) {
	...
	if (numbers.first !== null && numbers.second !== null && numbers.operation) {
		...
		if (numbers.result.toString().length > 13)
...
function processCalculation(numbers) {
	...
	switch (numbers.operation) {
		...
		case '-':
			if (numbers.result)
				// handle having a result from previous calculations as first operand
				return subtract(numbers.result, numbers.first);
			else return subtract(numbers.first, numbers.second);
		case '*':
		...
		case '/':
			...
			} else return divide(numbers.first, numbers.second, numbers);
	}
}
...
function handleKeyboardInput(numbers, inputKey) {
	...
	if (charTyped.match(onlyNumbers)) processParsedNumber(numbers, inputKey);
	else if (charTyped.match(onlyOperator)) assignOperator(numbers, charTyped);
	else if (charTyped === '=' || charTyped === 'Enter') equalResult(numbers);
	else if (charTyped === 'Backspace') undoInput(numbers);
	else if (charTyped === '.') addDecimal(numbers);
}
